...
== Debugging Help &ndash; Drawing Highlighted and Transparent
...
`%`: Transparent &ndash; draw an object in a translucent gray so you can see through it. This also omits the object from the rendered STL file.
...
=== Safely &ldquo;Drilling&rdquo; Holes
...
head. Let&lsquo;s create the model in several steps.
...
The far end should be rounded, though, so we&lsquo;ll shorten the
...
I&lsquo;ve highlighted the cylinder to show how the objects fit
together.

image:handle2.png[Rectangular handle with round end, 225, 92]

And we need a hole in the end, which I&rsquo;ve highlighted below.
